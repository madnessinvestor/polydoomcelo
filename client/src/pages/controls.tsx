...
const Key = ({ label, action, active, className = "" }: { label: string, action: string, active?: boolean, className?: string }) => (
  <div className={`flex flex-col items-center gap-1 ${className}`}>
    ...
    <span className="text-[10px] uppercase font-black tracking-tighter text-violet-400/70 text-center leading-none max-w-[60px] font-pixel-content">
      {action}
    </span>
  </div>
);
...
const ArrowKey = ({ dir, action, active }: { dir: 'up' | 'down' | 'left' | 'right', action: string, active?: boolean }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-[9px] uppercase font-black tracking-tighter text-violet-400/70 font-pixel-content">{action}</span>
    </div>
  );
};
        ...
        <CardContent className="p-8 space-y-12">
          ...
          <div className="space-y-12">
            ...
            <div className="flex flex-col items-center gap-6 bg-violet-950/10 p-8 rounded-lg border border-violet-500/10">
              <h3 className="text-violet-500 font-black uppercase text-sm tracking-[0.4em] font-pixel-label">Items Keys</h3>
            ...
            {/* Category: Basic Keys */}
            <div className="flex flex-col items-center gap-6 bg-violet-950/10 p-8 rounded-lg border border-violet-500/10">
              <h3 className="text-violet-500 font-black uppercase text-sm tracking-[0.4em] font-pixel-label">Basic Keys</h3>
